...
<div class="toolbar-nav">
	<div class="row">
		<div class="col-md-6 ">
			@if($access['is_add'] ==1)
	   		<a href="{{ url('vacancy/'.$id.'/edit?return='.$return) }}" class="tips btn btn-default btn-sm  " title="{{ __('core.btn_edit') }}"><i class="fa  fa-pencil"></i></a>
			@endif
			<a href="{{ url('vacancy?return='.$return) }}" class="tips btn btn-default  btn-sm  " title="{{ __('core.btn_back') }}"><i class="fa  fa-times"></i></a>		
		</div>
		<div class="col-md-6 text-right">			
	   		<a href="{{ ($prevnext['prev'] != '' ? url('vacancy/'.$prevnext['prev'].'?return='.$return ) : '#') }}" class="tips btn btn-default  btn-sm"><i class="fa fa-arrow-left"></i>  </a>	
			<a href="{{ ($prevnext['next'] != '' ? url('vacancy/'.$prevnext['next'].'?return='.$return ) : '#') }}" class="tips btn btn-default btn-sm "> <i class="fa fa-arrow-right"></i>  </a>					
		</div>	
		...
		<table class="table table-striped table-bordered " >
			<tbody>	
					...
					<tr>
						...
						<td>{{ SiteHelpers::formatLookUp($row->category,'category','1:tbl_role_catgory:id:category_name') }} </td>
					...
					<tr>
						<td width='30%' class='label-view text-right'>{{ SiteHelpers::activeLang('Vacancy', (isset($fields['vacancy']['language'])? $fields['vacancy']['language'] : array())) }}</td>
						<td>{{ $row->vacancy}} </td>
						
					</tr>
				
					<tr>
						<td width='30%' class='label-view text-right'>{{ SiteHelpers::activeLang('Employment Tatus', (isset($fields['employment_tatus']['language'])? $fields['employment_tatus']['language'] : array())) }}</td>
						<td>{{ $row->employment_tatus}} </td>
					...
					<tr>
						<td width='30%' class='label-view text-right'>{{ SiteHelpers::activeLang('Job Location', (isset($fields['job_location']['language'])? $fields['job_location']['language'] : array())) }}</td>
					...
					<tr>
						<td width='30%' class='label-view text-right'>{{ SiteHelpers::activeLang('Company Details', (isset($fields['company_details']['language'])? $fields['company_details']['language'] : array())) }}</td>
						<td>{{ $row->company_details}} </td>
