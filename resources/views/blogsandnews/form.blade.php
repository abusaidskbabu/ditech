...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'blogsandnews?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> Blogs And News</legend>
				{!! Form::hidden('id', $row['id']) !!}					
				           ...
				           >{{ $row['heading'] }}</textarea> 
									  ...
									  <div class="form-group row  " >
										<label for="Post By" class=" control-label col-md-4 text-left"> Post By </label>
										<div class="col-md-6">
										  <input  type='text' name='post_by' id='post_by' value='{{ $row['post_by'] }}' 
				...
				<div class="input-group input-group-sm m-b" style="width:150px !important;">
					...
					<div class="input-group-append">
						...
						<div class="image-preview preview-upload">
							{!! SiteHelpers::showUploadedFile( $row["image"],"/uploads/images/news") !!}
						</div>
					...
					<?php $extra1 = explode(',',$row['extra1']);
					$extra1_opt = array( 'news' => 'News' ,  'blog' => 'Blog' , ); ?>
					<select name='extra1' rows='5'   class='select2 '  > 
						<?php 
						foreach($extra1_opt as $key=>$val)
						{
							echo "<option  value ='$key' ".($row['extra1'] == $key ? " selected='selected' " : '' ).">$val</option>"; 						
						}						
						?></select> 
										 </div> 
										 <div class="col-md-2">
									  ...
									  <div class="form-group row  " >
										<label for="Extra2" class=" control-label col-md-4 text-left"> Extra2 </label>
										<div class="col-md-6">
										  <input  type='text' name='extra2' id='extra2' value='{{ $row['extra2'] }}' 
						     class='form-control form-control-sm ' /> 
										 </div> 
										 <div class="col-md-2">
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('blogsandnews/comboselect?filter=status:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("blogsandnews/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
